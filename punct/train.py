...
def train():
    ...
    for fold, (train_idx, val_idx) in enumerate(splitter.split(all_dataset, all_label), start=1):
        ...
        run = wandb.init(project='klue', entity='quarter100',
                         name=f'{MODEL_NAME}_punct_{fold}')
        ...
        save_dir = f'/opt/ml/code/results/{MODEL_NAME}_pucnt/{str(fold)}'
        ...
        trainer.train()
        model.save_pretrained(f'/opt/ml/code/best_model/xlm_punct_{str(fold)}')
        run.finish()
