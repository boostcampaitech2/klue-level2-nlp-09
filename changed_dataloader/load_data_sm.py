...
def load_data(path):
    ...
    for i, [x, y, z] in enumerate(zip(data['subject_entity'], data['object_entity'], data['sentence'])):
        ...
        if sub_i[0] < obj_i[0]:
            z = z[:sub_i[0]] + '[SUB]' + \
                z[sub_i[0]: sub_i[1]+1] + '[/SUB]' + z[sub_i[1]+1:]
            z = z[:obj_i[0]+11] + '[OBJ]' + z[obj_i[0] +
                                              11: obj_i[1]+12] + '[/OBJ]' + z[obj_i[1]+12:]
        else:
            z = z[:obj_i[0]] + '[OBJ]' + \
                z[obj_i[0]: obj_i[1]+1] + '[/OBJ]' + z[obj_i[1]+1:]
            z = z[:sub_i[0]+11] + '[SUB]' + z[sub_i[0] +
                                              11: sub_i[1]+12] + '[/SUB]' + z[sub_i[1]+12:]
