...
class Preprocess:
  def __init__(self, CSV_PATH):
    ...
    self.df= self.load_data(self.data)
  ...
  def load_data(self, data):
    ...
    for i, [x, y, z] in enumerate(zip(self.data['subject_entity'], self.data['object_entity'], self.data['sentence'])):
    ...
    df= pd.DataFrame({'id': self.data['id'], 'sentence' : sentence, 'subject_entity': sub_entity, 'object_entity': obj_entity,
                            'subject_type': sub_type, 'object_type': obj_type, 'label': self.data['label'],
...
if __name__ == '__main__':
  ...
  preprocess= Preprocess(TRAIN_PATH)
  # print(preprocess.df)
