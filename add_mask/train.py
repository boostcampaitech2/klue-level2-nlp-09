...
def get_config():
    ...
    parser.add_argument('--train_path', type= str, default= '/opt/ml/dataset/train/train_revised.csv',
                        help='train csv path (default: /opt/ml/dataset/train/dataset/train/train_revised.csv')
...
def train(args):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_dataset, all_label)):
        ...
        trainset= Dataset(masked_train, train_label)
        ...
        run= wandb.init(project= 'klue', entity= 'quarter100', name= 'masked-punct-star-'+'fold'+str(fold))
        trainer.train()
