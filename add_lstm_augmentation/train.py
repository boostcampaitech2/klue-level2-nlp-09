...
def train(args):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_dataset, all_label)):
        ...
        run= wandb.init(project= 'klue', entity= 'quarter100', name= f'jay_kr_kfold{fold}_lstm_punc_add_type_single')
        print(f'fold: {fold} start!')
        train_dataset= all_dataset.iloc[train_idx]
        augmented_train_dataset = easy_data_augmentation(train_dataset)
        #augmented_data.to_csv('swap.csv',index=False)
        #augmentation이 잘 되었는지 check하는 용도
        val_dataset= all_dataset.iloc[val_idx]

        train_label= preprocess.label_to_num(augmented_train_dataset['label'].values)
        val_label= preprocess.label_to_num(val_dataset['label'].values)

        tokenized_train, token_size= preprocess.tokenized_dataset(augmented_train_dataset, tokenizer)
